...
export class AuditService implements AuditServiceInterface {
    ...
    async logAction(
        ...
        try {
            const auditData = {
                ...
                old_status: data.oldValues?.status as string || null,
                new_status: data.newValues?.status as string || null,
                old_values: data.oldValues ? AuditMapper.sanitizeAuditValues(data.oldValues) : null,
                new_values: data.newValues ? AuditMapper.sanitizeAuditValues(data.newValues) : null,
                details: {
                    ...data.metadata,
    ...
    private async identifySecurityEvents(
        ...
        try {
            ...
            // Unusual access patterns (multiple IPs for same user)
    ...
    private identifyComplianceViolations(
        ...
        const unauthorizedAttempts = auditData.filter(log =>
            log.metadata?.error?.includes('unauthorized') ||
            log.metadata?.error?.includes('forbidden')
        ).length;
